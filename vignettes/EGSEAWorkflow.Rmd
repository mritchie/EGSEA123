...
Gene set enrichment analysis highlights the biological processes altered in genomic data sets. 
The Bioconductor project hosts over 80 software packages capable of gene set analysis, most of which search for enriched signatures amongst differentially regulated genes to reveal higher level biological themes.
With so many different methods on offer, choosing the best algorithm and visualization approach can be challenging. 
This workflow demonstrates use of the **EGSEA** package that is intended to solve this problem by combining the results from up to 12 prominent gene set testing algorithms to obtain a consensus ranking with biologically relevant results.
It shows how **EGSEA** can extend a **limma**-based differential expression analysis for RNA-seq and microarray data using experiments that profile 3 distinct cell populations important for studying the origins of breast cancer. 
...
# Acknowledgements
The authors thank Dr Alexandra Garnham for reading through this workflow article. This material was first trialled in a workshop at the BioC 2017 conference at the Dana Farber Cancer Institute (Boston, MA, 28 July 2017) and we the thank the participants at this workshop for their feedback.
